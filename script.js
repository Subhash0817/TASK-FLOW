...
function getFiltered() {
  ...
  if (searchText)
    list = list.filter((t) => t.text.toLowerCase().includes(searchText.toLowerCase()));
  return list;
}
...
(function () {
  const picker = $("timePicker"),
    panel = $("timePanel");
  const hourDisp = $("timeHour"),
    minDisp = $("timeMinute");
  const panelHour = $("panelHour"),
    panelMin = $("panelMinute");
  ...
  picker.addEventListener("click", (e) => {
    if (
      e.target.classList.contains("time-set-btn") ||
      e.target.classList.contains("time-clear-btn")
    )
      return;
  ...
  $("timeSetBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    hourDisp.textContent = pad(h);
    minDisp.textContent = pad(m);
...
(function () {
  const picker = $("datePicker"),
    panel = $("datePanel");
  const dayDisp = $("dateDay"),
    monDisp = $("dateMonth"),
    yrDisp = $("dateYear");
  const panelDay = $("panelDay"),
    panelMon = $("panelMonth"),
    panelYr = $("panelYear");
  ...
  $("dateSetBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    dayDisp.textContent = pad(d);
    monDisp.textContent = pad(mo);
    yrDisp.textContent = y;
...
loginBtn.addEventListener("click", async () => {
  ...
  if (response.ok) {
    ...
    notificationsPage.classList.add("hide");
